...
object LevelsMode {
    ...
    /**
     * Level 11 — The Lattice: a sparse, staggered field of single-cell pillars on
     * alternate rows (the rows between are clear), spaced five columns apart and
     * offset every other pillar row so the slalom staggers. It is generated by
     * tiling the top-left quadrant and mirroring every pillar into the other three,
     * so the field reads **4-fold symmetric on any board size** (a single periodic
     * grid can't stay mirror-symmetric on even dimensions, which looked lopsided).
     * Wide lanes keep it readable - hard but fair - and the board stays connected.
     */
    private fun lattice(b: BoardDimensions): Set<Position> {
        val stepX = 5
        val stepY = 2
        val offset = 2 // half-step shift that staggers alternate pillar rows
        val midX = (b.width - 1) / 2
        val midY = (b.height - 1) / 2
        return buildSet {
            var rowIndex = 0
            var y = 2
            while (y <= midY) {
                val phase = if (rowIndex % 2 == 0) 0 else offset
                var x = 2 + phase
                while (x <= midX) {
                    // Mirror into all four quadrants for size-independent symmetry.
                    add(Position(x, y))
                    add(Position(b.width - 1 - x, y))
                    add(Position(x, b.height - 1 - y))
                    add(Position(b.width - 1 - x, b.height - 1 - y))
                    x += stepX
                }
                y += stepY
                rowIndex++
