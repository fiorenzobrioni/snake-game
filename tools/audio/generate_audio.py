#!/usr/bin/env python3
"""Synthesize the game's original sound effects (SFX).
...
Note: the looping background music (``music_menu`` / ``music_game``) is no longer
synthesized here — it is provided as external tracks generated by Google Gemini
(see ``docs/CREDITS.md``). This script only produces the SFX.
"""
...
# --- Note table (shared by the melodic SFX) -------------------------------
...
def main() -> None:
    ...
    random.seed(0x5EED)
    print(f"Generating SFX into {RES_RAW}")
    sfx_eat()
    sfx_shrink()
    sfx_mystery()
    sfx_game_over()
