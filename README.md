...
> 🧱 **Current status:** Phase 4 (audio) complete, on top of Phase 3 (pro UI/UX) and the
> **Phase 2.5 food system** — a polished, playable Snake with an animated main menu and settings,
> persistent highscores, a responsive full-screen board, control schemes (swipe by default, plus
> two-button relative and D-pad), smooth interpolated motion, particles, screen shake, a pause blur,
> and now **music + sound effects** with master/music/SFX volume controls. The food layer adds two
> **categories** (grow / shrink) with magnitude tiers, **maxi** sizes, a **mystery "?"** piece, a
> **time-gated** progression that ramps mid-session, and a **combo** score multiplier. AGSL shaders
...
> The special power-ups (earthquake, explosion, speed, ghost, freeze, jackpot) and AGSL shader effects
> come online in later phases of the [roadmap](ROADMAP.md).
...
[`tools/audio/generate_audio.py`](tools/audio/generate_audio.py) — re-run it to reproduce them.
