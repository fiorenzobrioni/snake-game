...
- **Local verification during implementation**: while working, run the lightest check that covers the change (e.g. `./gradlew compileDebugKotlin`, `./gradlew testDebugUnitTest`, or `./gradlew lintDebug`) to confirm the code is correct before committing. A full `assembleDebug` is not required just to verify compilation - CI does the authoritative build.
- **Debug APK delivered in chat (convenience)**: at the end of a roadmap **phase** (or whenever the user asks for something to try), if an Android SDK is available in the environment, build the debug APK (`./gradlew assembleDebug`) and deliver the file (`app/build/outputs/apk/debug/app-debug.apk`) to the user **in chat** so they can install it immediately, without waiting for or navigating to the CI artifact. This in-chat build is a convenience copy, not the source of truth. If the build environment has no Android SDK and one cannot be provisioned, say so explicitly instead of silently skipping, and point the user to the CI `app-debug-apk` artifact instead. Never commit the APK - it is a build artifact.
