...
- **Release signing** (since Aug 2026): the real keystore lives OUTSIDE the repo (`C:\Fiorenzo\keys\snake-release.jks` on the dev machine); the `release` signingConfig is created only when the four `SNAKE_KEYSTORE*` properties are all set - from `~/.gradle/gradle.properties` locally, from `ORG_GRADLE_PROJECT_*` env vars (GitHub Secrets `KEYSTORE_BASE64`/`KEYSTORE_PASSWORD`/`KEY_ALIAS`/`KEY_PASSWORD`) in CI - and wins whenever configured. On an unconfigured checkout the release build is unsigned; `-PsignReleaseWithDebugKey` signs it with the committed debug key so per-push CI produces an installable minified build for smoke tests (R8 breakage shows up nowhere else). `release.yml` fires on version tags: tests + lint, then the real-key APK and its R8 mapping published as the GitHub Release, with the body taken from `docs/release-notes/<tag lowercased>.md` (notes are versioned in the repo since 3.1.0, like in the sibling repo saldo; the workflow fails if the file is missing and rewrites relative links to absolute URLs pinned to the tag). Writing that notes file is part of every release change, together with the version bump. The release APK's `applicationId` is `com.callbackdev.snake` (no `.debug` suffix), so the first real-key release installs **alongside** the debug-signed app people have from 3.0.0 - a one-off fresh install, same as the 2.0.0 → 3.0.0 id move.
