...
Follow-up (same day): tuned the **3D Fixed** view per feedback - brought the camera ~2x closer
(halved the `FIXED_BACK_*`/`FIXED_HEIGHT_*` constants in `ChaseCam.kt`) so the board reads large
instead of small-and-whole, and made steering **absolute** there (it is north-locked, so it should
behave like 2D). Added `GameViewModel.relativeSteering` (`threeDActive && !viewMode.fixedNorth`) and
routed `onSwipe` + `ControlRegion.forceRelative` through it; the rotating 3D follow view still uses
relative turns.
