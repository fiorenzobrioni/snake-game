...
- **Even mode tiles.** "Daily Challenge" / "Random Challenge" wrapped to two lines, making those tiles
  taller than the one-line "Custom" tile beside them. Added short single-word tile labels
  (`menu_daily_short` = "Daily", `menu_random_short` = "Random") and forced the `MenuTile` label to one line
  (`maxLines = 1`, ellipsis), so the three Modes tiles are now identical in width and height.
