...
### 2026-06-21 - Living atmospheric background (shader)

Reworked `Shaders.BACKGROUND` from a simple gradient + two glows into a richer, skin-aware atmosphere
shared by the board and the menus.

- **Shader.** Now layers a drifting fbm **nebula** (clouds crossfading between two accents), two
  **breathing bloom glows**, a slow diagonal **light sweep**, a stronger **vignette** and ordered
  **dithering** (kills banding on the dark gradient) over the gradient. Colours are uniforms
  (`topColor`/`bottomColor`/`glowA`/`glowB`) so every skin - and the menu - gets its own palette, and an
  `intensity` uniform scales the additive layers. A centre-calm mask keeps the atmosphere quietest where
  the action / menu title sit and lets it bloom toward the edges, protecting contrast.
- **Board.** `drawBoardBackground` feeds the skin's `boardTop`/`boardBottom` + `headGlow`/`boardBorder`
  accents at a deliberately low `BOARD_BACKGROUND_INTENSITY` (0.55) so the board stays an atmosphere, not
  a distraction. The board background is now properly **skin-tinted** (previously the gradient colours
  were hard-coded and ignored the skin).
- **Menu.** `AnimatedShaderBackground` runs the same shader at full intensity with a brand palette
  (deep blue-black gradient, teal-green + blue accents).

Verified: `./gradlew compileDebugKotlin` + `testDebugUnitTest` green; debug APK built. The AGSL itself
compiles on-device only (no emulator here) - every construct reuses ones already shipped, but worth an
eyeball on hardware.
